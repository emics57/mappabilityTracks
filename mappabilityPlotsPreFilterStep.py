...
for j in readLengths:
    ...
    # filtering step for confident multimapping reads 
    notOneVals = value_counts[value_counts > 1].index.tolist()
    notOneDF = df[df[0].isin(notOneVals)]
    notOneDF = notOneDF[notOneDF[4] != None] # drop rows with None as asScore
    notOneDF[4] = notOneDF[4].astype(int)
    topTwoIndices = notOneDF.groupby(0)[4].apply(lambda x: x.nlargest(2).index).explode()
    topTwoDF = notOneDF.loc[topTwoIndices]
    topTwoDF['Ratio'] = topTwoDF.groupby(0)[4].apply(lambda x: x.shift(-1) / x)
    lessThan99DF = topTwoDF[topTwoDF['Ratio'] <= 0.99] # keep only multimapping reads with AS score less than 0.99
    readsToDeduplicate = lessThan99DF[0].tolist()
    subset_to_deduplicate = df[df[0].isin(readsToDeduplicate)]
    subset_untouched = df[~df[0].isin(readsToDeduplicate)]
    subset_deduplicated = subset_to_deduplicate[0].drop_duplicates()
    result_df = pd.concat([subset_deduplicated, subset_untouched], ignore_index=True)

    # value counts after filtering step
    value_counts = result_df[0].value_counts()
    ...
    unique_mapped_df = result_df[result_df[0].isin(unique_mapped_values)]
    ...
    multi_mapped_df = result_df[result_df[0].isin(multi_mapped_values)]
